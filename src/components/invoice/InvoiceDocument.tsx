...
// Top Hills — Invoice A4-portrait (1080×1528). Pixel-match brand spec.
// Dua variant: 'krem' (Krem Klasik) & 'pita' (Pita Emas). Inline styles untuk
// fidelity & supaya aman saat di-capture (html2canvas).
...
interface Props {
  ...
  showStamp?: boolean;
  showQR?: boolean;
  forExport?: boolean; // sembunyikan tombol salin saat capture
  copied?: string | null;
  onCopyRek?: () => void;
  onCopyTotal?: () => void;
}
...
export function InvoiceDocument({
  ...
  return (
    <div
      style={{
        width: 1080, minHeight: 1528, padding: 46, position: 'relative',
        fontFamily: SANS, overflow: 'hidden',
        background: 'radial-gradient(120% 82% at 18% 0%, #F4ECDC 0%, #EBE2CF 52%, #E2D8C2 100%)',
      }}
    >
      {/* gold glows */}
      ...
      <div style={{ position: 'relative', borderRadius: 26, overflow: 'hidden', background: 'linear-gradient(160deg, rgba(255,253,247,.74), rgba(248,242,231,.56))', backdropFilter: 'blur(26px)', WebkitBackdropFilter: 'blur(26px)', border: '1px solid rgba(156,122,46,.30)', boxShadow: '0 42px 92px -34px rgba(120,96,40,.40), inset 0 1px 0 rgba(255,255,255,.65)' }}>
          ...
          <div style={{ background: 'linear-gradient(135deg,#A6802F 0%,#8A6A24 55%,#6E551C 100%)', padding: '34px 56px 32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: 'inset 0 -1px 0 rgba(0,0,0,.15)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 18 }}>
              ...
              <div style={{ borderLeft: '1px solid rgba(243,236,221,.4)', paddingLeft: 18 }}>
                <div style={{ fontSize: 11, letterSpacing: 3, color: 'rgba(243,236,221,.85)', fontWeight: 600 }}>KOST &amp; PENGINAPAN</div>
                <div style={{ fontSize: 13, color: 'rgba(243,236,221,.7)', marginTop: 7 }}>Limau Manis, Pauh — Padang</div>
              </div>
            </div>
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontFamily: SERIF, fontWeight: 600, fontSize: 50, letterSpacing: 8, lineHeight: .9, color: '#F6EFDF' }}>INVOICE</div>
              {inv.tag && <div style={{ display: 'inline-block', marginTop: 10, padding: '5px 14px', border: '1px solid rgba(246,239,223,.55)', borderRadius: 20, fontSize: 10, letterSpacing: 2.5, color: '#F6EFDF', fontWeight: 700 }}>{inv.tag}</div>}
              <div style={{ display: 'inline-grid', gridTemplateColumns: 'auto auto', gap: '6px 16px', marginTop: 14, textAlign: 'right' }}>
                {metaRows.map(([k, v]) => (
        ...
        <div style={{ position: 'relative', padding: '46px 56px 42px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 40 }}>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 10.5, letterSpacing: 2.4, color: GOLD, fontWeight: 700, marginBottom: 11 }}>DITAGIHKAN KEPADA</div>
              <div style={{ fontFamily: SERIF, fontSize: 26, fontWeight: 600, color: '#2C2620', lineHeight: 1.1 }}>{inv.customer.name}</div>
              {inv.customer.phone && <div style={{ fontSize: 13.5, color: '#7A7164', marginTop: 6, fontVariantNumeric: 'tabular-nums' }}>{inv.customer.phone}</div>}
              <div style={{ fontSize: 13.5, color: '#7A7164', marginTop: 3 }}>{inv.customer.kind}</div>
            </div>
            <div style={{ flex: 1, textAlign: 'right' }}>
              <div style={{ fontSize: 10.5, letterSpacing: 2.4, color: GOLD, fontWeight: 700, marginBottom: 11 }}>DETAIL PEMESANAN</div>
              <div style={{ fontSize: 15, color: '#2C2620', fontWeight: 700 }}>{inv.booking.room}</div>
              {inv.booking.period && <div style={{ fontSize: 13.5, color: '#7A7164', marginTop: 6 }}>{inv.booking.period}</div>}
              <div style={{ fontSize: 13.5, color: '#7A7164', marginTop: 3 }}>Check-in 13.00 · Check-out 12.00</div>
            </div>
          </div>
          ...
          <div style={{ marginTop: 30 }}>
            ...
            {inv.items.map((row, i) => (
              <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 64px 168px 180px', alignItems: 'center', padding: '18px 4px', borderBottom: '1px solid rgba(60,52,40,.10)' }}>
                <div>
                  <div style={{ fontSize: 16.5, color: '#2C2620', fontWeight: 600 }}>{row.desc}</div>
                  {row.note && <div style={{ fontSize: 12.5, color: '#8A8170', marginTop: 3 }}>{row.note}</div>}
                </div>
                <div style={{ fontSize: 15, color: '#5A5446', textAlign: 'center', fontVariantNumeric: 'tabular-nums' }}>{row.qty}</div>
                <div style={{ fontSize: 15, color: '#5A5446', textAlign: 'right', fontVariantNumeric: 'tabular-nums' }}>{rp(row.price)}</div>
                <div style={{ fontSize: 15, color: '#2C2620', textAlign: 'right', fontWeight: 600, fontVariantNumeric: 'tabular-nums' }}>{rp(row.qty * row.price)}</div>
              </div>
            ))}
          </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 26 }}>
              <div style={{ width: 430, borderRadius: 16, padding: '22px 26px', background: 'linear-gradient(135deg,#A6802F,#7C5F22)', boxShadow: '0 16px 36px -16px rgba(120,96,40,.6)' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                  <span style={{ fontSize: 11, letterSpacing: 2, color: 'rgba(246,239,223,.9)', fontWeight: 700 }}>{balanceLabel}</span>
                  <span style={{ fontFamily: SERIF, fontWeight: 700, fontSize: 40, lineHeight: 1, color: '#FBF6EC', fontVariantNumeric: 'tabular-nums' }}>{rp(balanceVal)}</span>
                </div>
                {!fullyPaid && !forExport && (
                  <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 14 }}>
          ...
          <div style={{ display: 'flex', gap: 22, marginTop: 34, alignItems: 'stretch' }}>
            <div style={{ flex: 1, border: '1px solid rgba(156,122,46,.26)', borderRadius: 16, padding: '22px 24px', background: 'rgba(156,122,46,.05)' }}>
              <div style={{ fontSize: 10.5, letterSpacing: 2.4, color: GOLD, fontWeight: 700, marginBottom: 14 }}>PEMBAYARAN · TRANSFER</div>
              <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '9px 16px' }}>
                <span style={{ fontSize: 13, color: '#8A8170' }}>Bank</span>
                <span style={{ fontSize: 14, color: '#2C2620', fontWeight: 600, textAlign: 'right' }}>{identity.bankName}</span>
                <span style={{ fontSize: 13, color: '#8A8170', alignSelf: 'center' }}>No. Rekening</span>
                <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 8 }}>
                  <span style={{ fontSize: 14, color: '#2C2620', fontWeight: 600, fontVariantNumeric: 'tabular-nums', letterSpacing: 1 }}>{identity.accountNo}</span>
                  {!forExport && <CopyBtn small onClick={onCopyRek} done={copied === 'rek'} doneText="Tersalin ✓" text="Salin" theme="gold" />}
                </span>
                <span style={{ fontSize: 13, color: '#8A8170' }}>Atas Nama</span>
                <span style={{ fontSize: 14, color: '#2C2620', fontWeight: 600, textAlign: 'right' }}>{identity.accountName}</span>
              </div>
              <div style={{ fontSize: 12, color: '#8A8170', marginTop: 16, lineHeight: 1.55 }}>Kirim bukti transfer <span style={{ color: '#5A5446' }}>asli</span> ke WhatsApp Resmi · {identity.waResmi} untuk diverifikasi admin.</div>
            </div>
            {showQR && (
              <div style={{ width: 172, flex: 'none', border: '1px solid rgba(156,122,46,.26)', borderRadius: 16, padding: 16, background: 'rgba(156,122,46,.05)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                {identity.qrisBase64 ? (
                  ...
                  <div style={{ width: 120, height: 120, borderRadius: 10, background: 'repeating-linear-gradient(45deg,#9C7A2E 0 6px, #EFE7D6 6px 12px)', opacity: .55, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <span style={{ fontFamily: SANS, fontSize: 10, letterSpacing: 1.5, color: '#F6EFDF', background: GOLD, padding: '3px 7px', borderRadius: 4, fontWeight: 700 }}>QRIS</span>
                  </div>
                )}
                <div style={{ fontSize: 11, letterSpacing: 1.6, color: '#7A7164', marginTop: 12, textAlign: 'center' }}>SCAN UNTUK BAYAR</div>
              </div>
            )}
          </div>
          ...
          {/* TERMS */}
          <div style={{ marginTop: 30 }}>
            <div style={{ fontSize: 10.5, letterSpacing: 2.4, color: GOLD, fontWeight: 700, marginBottom: 12 }}>SYARAT &amp; KETENTUAN</div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', gap: 40 }}>
            <div style={{ maxWidth: 360 }}>
              <div style={{ fontFamily: SERIF, fontStyle: 'italic', fontSize: 22, color: '#4A443C', lineHeight: 1.3 }}>Terima kasih telah mempercayai Top Hills 🌸</div>
              <div style={{ fontSize: 12, color: '#8A8170', marginTop: 10, lineHeight: 1.5 }}>Pembayaran dianggap sah setelah bukti diverifikasi oleh admin resmi.</div>
            </div>
            <div style={{ textAlign: 'right', minWidth: 280 }}>
              <div style={{ fontSize: 10.5, letterSpacing: 2.4, color: '#9C8A6A', fontWeight: 700 }}>HORMAT KAMI</div>
              ...
              <div style={{ fontFamily: SERIF, fontSize: 21, fontWeight: 600, color: '#2C2620', marginTop: 10 }}>{identity.ownerName}</div>
              <div style={{ fontSize: 12, color: '#7A7164', letterSpacing: .5, marginTop: 2 }}>{identity.ownerTitle}</div>
            </div>
          </div>
          ...
          {showStamp && fullyPaid && (
            <div style={{ position: 'absolute', bottom: 120, left: '51%', transform: 'translateX(-50%) rotate(-13deg)', pointerEvents: 'none' }}>
              <div style={{ border: '3px double #9C7A2E', borderRadius: 14, padding: '12px 28px 10px', textAlign: 'center', boxShadow: '0 0 26px rgba(156,122,46,.16)', background: 'rgba(156,122,46,.05)' }}>
                ...
                <div style={{ fontSize: 11, letterSpacing: 5, color: '#9C7A2E', marginTop: 5, opacity: .85 }}>PAID IN FULL</div>
                <div style={{ fontSize: 10, letterSpacing: 1.5, color: '#A6802F', marginTop: 5, opacity: .7 }}>{inv.date}</div>
              </div>
            </div>
          )}
          {showStamp && !fullyPaid && (
...
function hdr(align: 'left' | 'center' | 'right' = 'left'): React.CSSProperties {
  return { fontSize: 10.5, letterSpacing: 2, color: GOLD, fontWeight: 700, textAlign: align };
}
function FragmentRow({ k, v, kColor, vColor }: { k: string; v: string; kColor: string; vColor: string }) {
  return (
    <>
      <div style={{ fontSize: 10, letterSpacing: 1.6, color: kColor, alignSelf: 'center' }}>{k}</div>
      <div style={{ fontSize: 13.5, color: vColor, fontWeight: 600, fontVariantNumeric: 'tabular-nums' }}>{v}</div>
    </>
  );
}
function TotRow({ label, value, cream }: { label: string; value: string; cream?: boolean }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', padding: cream ? '5px 0' : '7px 4px' }}>
      <span style={{ fontSize: 14, color: cream ? 'rgba(246,239,223,.78)' : '#7A7164' }}>{label}</span>
      <span style={{ fontSize: 14, color: cream ? '#F6EFDF' : '#3A332A', fontVariantNumeric: 'tabular-nums' }}>{value}</span>
    </div>
  );
}
function Term({ children }: { children: React.ReactNode }) {
  return (
    <div style={{ display: 'flex', gap: 9, fontSize: 12.5, color: '#8A8170', lineHeight: 1.5 }}>
  ...
  return (
    <button
      ...
      style={{
        ...
        color: cream ? '#F6EFDF' : GOLD, fontSize: small ? 11 : 12, fontWeight: 700, letterSpacing: .4,
        padding: small ? '4px 9px' : '8px 14px', borderRadius: small ? 8 : 9, cursor: 'pointer',
